...
function [ yOut, uOut, normFout, objFun, info] = run_example_affine( H, z, measurePLQ, processPLQ, linTerm, params )
...
kktSystemFunc = @kktSystemBarrierAffine;
ipSolverFunc  = @ipSolverBarrierAffine;
...
%     end
    ...
    yIn   = params.E\params.e; %ones(n, 1);
    
    zetaIn = ones(length(params.e),1);
    ...
    Fin = kktSystemFunc(linTerm, b, Bm, c, C, Mv, qIn, uIn, rIn, wIn, yIn,zetaIn, params);    
    info.pcgIter = [];
    params.info = info;
    [yOut, uOut, qOut, rOut, wOut, zetaOut, info] = ipSolverFunc(linTerm, b, Bm, c, C, Mv, qIn, uIn, rIn, wIn, yIn, zetaIn, params);
   % hist = info.primal;
    Fout = kktSystemFunc(linTerm, b, Bm, c, C, Mv, qOut, uOut, rOut, wOut, yOut, zetaOut, params);
