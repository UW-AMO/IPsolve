%% sine example
%Here the state x = [position; velocity] and we model the acceleration as
%the error. As acceleration impacts position and velocity this leads to a
%singular covariance matrix in the state equations
%
%We model the acceleration with white mean zero noise. This should work
%well in this case
%

gamma = 1;
%signalFunc = @(x) sin(-x);
signalFunc = @(x) exp(sin(4*x))
box = [-1,1]; % lower, upper 
numP = 4;
N     = 100;        % number of measurement time points
dt    = numP*2*pi / (4*N);  % time between measurement points
sigma =  .1;       % standard deviation of measurement noise
sigmaMod = 0.1;    % sigma we tell smoot
outliers = 1;

if(outliers)
    out = N*.1;        % percent of outliers
    mag = 5;            % outlier variance
else
    out = 0;
    mag = 0;
end

m     = 1;          % number of measurements per time point
n     = 2;          % number of states per time point
t       =  (1 : N) * dt; % set of times

rand('seed', 1234);
%x1_true = - cos(t);
x_true = signalFunc(t);

%x_true  = [ x1_true ; x2_true ];
%
% measurement values and model
gaussErrors  = sigma * randn(1, N);
% construct the outliers
outliers = zeros(1,N);
inds = randperm(N, out); % indices of outliers
outliers(inds) = mag*randn(1,out);
z       = x_true + gaussErrors + outliers;

R = sigma*sigma;
gain = [.5*dt*dt; dt];
Q = gain*gain';
%Q = gamma^2*Q;
Q = 50^2/12*Q;

G = [1 dt; 0 1];
H = [1 0];

[D, A, hatw] = SingularKalmanSetup(G, H, Q, R, [0;0], N, z');
[Dr Dc] = size(D);

params.E = A; 
params.e = hatw; 
params.eqFlag =1; % do we need this?
tic
[aff_soln] = run_example_affine(D, zeros(Dr,1), 'huber', [], [], params);
time = toc;

[xsoln, vsoln, wsoln] = extractor(m, n, N, aff_soln);

h_min   = 0;       % minimum horizontal value in plots
h_max   = dt*N;       % maximum horizontal value in plots
v_min   = min(x_true)*1.3 -1;    % minimum vertical value in plots
v_max   = max(x_true)*1.3 +.2;    % maximum vertical value in plots

plot(t', x_true, 'k', t', xsoln(1,2:end)', 'r--', t', z, 'ko', 'Linewidth', 2)
legend('truth', 'singular estimate','observed data');
axis([h_min, h_max, v_min, v_max]);



%% exp example with zero mean white acceleration
%here we will use the same setup as above but with an exp function. The
%model here should not work as well as the acceleration grows quickly


gamma = 1;
%signalFunc = @(x) exp(x);
signalFunc = @(x) exp(-x);
box = [-1,1]; % lower, upper 
numP = 1;
N     = 25;        % number of measurement time points
dt    = 2 / N;  % time between measurement points
sigma =  .1;       % standard deviation of measurement noise
sigmaMod = 0.1;    % sigma we tell smoot
outliers = 0;

if(outliers)
    out = N*.05;        % percent of outliers
    mag = 1;            % outlier variance
else
    out = 0;
    mag = 0;
end

m     = 1;          % number of measurements per time point
n     = 2;          % number of states per time point
t       =  (1 : N) * dt; % set of times

rand('seed', 1234);
%x1_true = - cos(t);
x_true = signalFunc(t);

%x_true  = [ x1_true ; x2_true ];
%
% measurement values and model
gaussErrors  = sigma * randn(1, N);
% construct the outliers
outliers = zeros(1,N);
inds = randperm(N, out); % indices of outliers
outliers(inds) = mag*randn(1,out);
z       = x_true + gaussErrors + outliers;

R = sigma*sigma;
gain = [.5*dt*dt; dt];
Q = gain*gain';
Q = gamma^2*Q;

G = [1 dt; 0 1];
H = [1 0];

[D, A, hatw] = SingularKalmanSetup(G, H, Q, R, [1;1], N, z');
[Dr Dc] = size(D);

params.E = A; 
params.e = hatw; 
params.eqFlag =1; % do we need this?
tic
[aff_soln] = run_example_affine(D, zeros(Dr,1), 'huber', [], [], params);
time = toc;

[xsoln, vsoln, wsoln] = extractor(m, n, N, aff_soln);

plot(t', x_true, 'k', t', xsoln(1,2:end)', 'r--', t', z, 'ko')
legend('truth', 'estimate (no acceleration)','observed data');


%% Exp example with autocorrelated (colored) noise
%Going to model the state error in acceleration (one dim) as w+k = w_k-1 + s_k
%where s_k is a zero mean white noise sequence
%
%In order to plug this into our model we will augment the state to be x =
%[position; velocity; w_k]
%
%we will thus start with x_0 = [1;1;1]


gamma = 1;
signalFunc = @(x) exp(x);
%signalFunc = @(x) exp(-x);
box = [-1,1]; % lower, upper 
numP = 1;
N     = 30;        % number of measurement time points
dt    = 2 / N;  % time between measurement points
sigma =  .1;       % standard deviation of measurement noise
sigmaMod = 0.1;    % sigma we tell smoot
outliers = 1;

if(outliers)
    out = N*.1;        % percent of outliers
    mag = 5;            % outlier variance
else
    out = 0;
    mag = 0;
end

m     = 1;          % number of measurements per time point
n     = 3;          % number of states per time point
t       =  (1 : N) * dt; % set of times

rand('seed', 1234);
%x1_true = - cos(t);
x_true = signalFunc(t);


%
% measurement values and model
gaussErrors  = sigma * randn(1, N);
% construct the outliers
outliers = zeros(1,N);
inds = randperm(N, out); % indices of outliers
outliers(inds) = mag*randn(1,out);
z       = x_true + gaussErrors + outliers;

R = sigma*sigma;

gain = [.5*dt*dt; dt; 1];
Q = gain*gain';
Q = 3.4*Q;


G = [1 dt .5*dt*dt; 0 1 dt; 0 0 1];

H = [1 0 0];

[D, A, hatw] = SingularKalmanSetup(G, H, Q, R, [1;1;1], N, z');
[Dr Dc] = size(D);

params.E = A; 
params.e = hatw; 
params.eqFlag =1; % do we need this?
tic
[aff_soln] = run_example_affine(D, zeros(Dr,1), 'huber', [], [], params);
time = toc;

[xsoln, vsoln, wsoln] = extractor(m, n, N, aff_soln);

plot(t', x_true, 'k', t', xsoln(1,2:end)', 'r--', t', z, 'ko', 'Linewidth', 2)
legend('truth', 'estimate with acceleration','observed data');


%% non-singular model
%note here the state is x=[velocity position]

clear all; close all; clc

signal = 'exp';
switch(signal)
    
    case{'sine'}
        gamma =  1;         % transition covariance multiplier
        signalFunc = @(x) sin(-x);
        box = [-1,1]; % lower, upper 
        numP = 4;
        N     = 100;        % number of measurement time points
        dt    = numP*2*pi / N;  % time between measurement points
        sigma =  .4;       % standard deviation of measurement noise
        sigmaMod = 0.1;    % sigma we tell smoother
        outliers = 1;
        if(outliers)
            out = N*.05;        % percent of outliers
            mag = 5;            % outlier variance
        else
            out = 0;
            mag = 0;
        end
        
        % box constraints for constrained estimates
        conA = [0 1; 0 -1];
        cona = [1; 1];
        
        measPLQ = 'huber';
        procPLQ = 'l2';
        
        
    case{'exp'}
        gamma = 0.1;
        mult = 4;
        signalFunc = @(x) exp(sin(mult*x));
        numP  = mult;         %
        N     = 100;        % number of measurement time points
        dt    = numP*2*pi / (N*mult);  % time between measurement points
        sigma =  .05;       % standard deviation of measurement noise
        sigmaMod = sigma;   % give nominal variance 
        outliers = 1;
        if(outliers)
            out = N*.1;        % percent of outliers
            mag = 5;            % outlier variance
        else
            out = 0;
            mag = 0;
        end
        
        
        measPLQ = 'huber';
        procPLQ = 'huber';
        
                % box constraints
        conA = [0 1; 0 -1];
        cona = [exp(1); -exp(-1)];

        
        
    otherwise
        error('unknown signal');
end

m     = 1;          % number of measurements per time point
n     = 2;          % number of measurements per time point
t       =  (1 : N) * dt; % set of times


max_itr = 30;      % maximum number of iterations
epsilon = 1e-5;    % convergence criteria

% smooth signal:

rand('seed', 1234);
%x1_true = - cos(t);
x_true = signalFunc(t);

% plotting parameters
h_min   = 0;       % minimum horizontal value in plots
h_max   = dt*N;       % maximum horizontal value in plots
v_min   = min(x_true)*1.3 -1;    % minimum vertical value in plots
v_max   = max(x_true)*1.3 +.2;    % maximum vertical value in plots

%x_true  = [ x1_true ; x2_true ];
%
% measurement values and model
gaussErrors  = sigma * randn(1, N);
% construct the outliers
outliers = zeros(1,N);
inds = randperm(N, out); % indices of outliers
outliers(inds) = mag*randn(1,out);
z       = x_true + gaussErrors + outliers;


%model set-up
H = [0 1];
G = [1, 0; dt, 1];
Q = gamma * [ dt , dt^2/2 ; dt^2/2 , dt^3/3 ];
R = sigma^2;


[D, A, hatw] = SingularKalmanSetup(G, H, Q, R, [0;0], N, z');

[Dr Dc] = size(D);

params.E = A; 
params.e = hatw; 
params.eqFlag =1; % do we need this?
tic
[aff_soln] = run_example_affine(D, zeros(Dr,1), 'huber', [], [], params);
time = toc;

[xsoln, vsoln, wsoln] = extractor(m, n, N, aff_soln);
hold on
plot(t', x_true, 'k', t', xsoln(2,2:end)', 'r--', 'Linewidth', 2)
plot(t', z, 'ko')
legend('truth', 'singular robust estimate','observed data');
axis([h_min, h_max, v_min, v_max]);



